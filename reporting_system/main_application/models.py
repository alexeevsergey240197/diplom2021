...
class GroupOfReports(models.Model):
    name = models.CharField('Наименованеи группы отчётов', max_length=20)


class Report(models.Model):
    status_choice = (('Сформирован', 'Сформирован'), ('Рассматривается', 'Рассматривается'), ('Доработать', 'Доработать'), ('Новый', 'Новый'))
...
class UserProfile(models.Model):
    role_choice = (('Поручитель отчётности', 'Поручитель отчётности'), ('Субъект отчётности', 'Субъект отчётности'), ('Администратор', 'Администратор'))
    user = models.OneToOneField(User, default="default user", on_delete=models.CASCADE)
    organisation = models.OneToOneField(Organisation, null=True, blank=True, on_delete=models.DO_NOTHING, verbose_name='Организация')
    ...
    role = models.CharField('Роль', max_length=20, choices=role_choice)
